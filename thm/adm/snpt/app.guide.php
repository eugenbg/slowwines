...
try
{
	...
	$oCRUD->table( TBL_APP_CNTNT_GUIDE );
	$oCRUD->order_by( 'title, _updated' );
	$oCRUD->table_name( 'Guide Editor' );
	...
	//  View.
	$oCRUD->columns( 'image, title, email, _active, _editor, _created, _updated' );
	$oCRUD->column_width( 'image', '75px' ); 
	$oCRUD->column_width( '_active, _editor', '100px' ); 
	$oCRUD->column_width( '_created, _updated', '150px' ); 
	$oCRUD->column_class( '_active, _editor, _created, _updated', 'align-center' );

	//  Overview Tab.
	$oCRUD->fields( 'title, email, image, _active', false, $oTrn->word( 'general.overview' ) );
