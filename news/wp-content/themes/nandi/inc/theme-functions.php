...
if ( ! function_exists( 'nandi_get_setting' ) ) {
	/**
	 * A wrapper function to get our settings.
	 *
	 */
	function nandi_get_setting( $setting ) {
		$nandi_settings = wp_parse_args(
			get_option( 'nandi_settings', array() ),
			nandi_get_defaults()
		);

		return $nandi_settings[ $setting ];
	}
}

if ( ! function_exists( 'nandi_get_layout' ) ) {
	/**
	...
	function nandi_get_layout() {
		...
		$layout = $nandi_settings['layout_setting'];

		// Get the individual page/post sidebar metabox value
		$layout_meta = ( isset( $post ) ) ? get_post_meta( $post->ID, '_nandi-sidebar-layout-meta', true ) : '';
		...
		if ( is_single() && ! $buddypress ) {
			$layout = null;
			$layout = $nandi_settings['single_layout_setting'];
		}
		...
		if ( is_home() || is_archive() || is_search() || is_tax() ) {
			$layout = null;
			$layout = $nandi_settings['blog_layout_setting'];
		}

		// Finally, return the layout
		return apply_filters( 'nandi_sidebar_layout', $layout );
	}
}

if ( ! function_exists( 'nandi_get_footer_widgets' ) ) {
	/**
	 ...
	 */
	function nandi_get_footer_widgets() {
		// Get current post
		global $post;
		...
		$widgets = $nandi_settings['footer_widget_setting'];

		// Get the individual footer widget metabox value
		$widgets_meta = ( isset( $post ) ) ? get_post_meta( $post->ID, '_nandi-footer-widget-meta', true ) : '';
		...
		// Finally, return the layout
		return apply_filters( 'nandi_footer_widgets', $widgets );
	}
}

if ( ! function_exists( 'nandi_show_excerpt' ) ) {
	/**
	 * Figure out if we should show the blog excerpts or full posts
	 *
	 */
	function nandi_show_excerpt() {
		// Get current post
		global $post;
		...
		$more_tag = apply_filters( 'nandi_more_tag', strpos( $post->post_content, '<!--more-->' ) );

		// Check the post format
		$format = ( false !== get_post_format() ) ? get_post_format() : 'standard';

		// Get the excerpt setting from the Customizer
		$show_excerpt = ( 'excerpt' == $nandi_settings['post_content'] ) ? true : false;
		...
		// Return our value
		return apply_filters( 'nandi_show_excerpt', $show_excerpt );
	}
}

if ( ! function_exists( 'nandi_show_title' ) ) {
	/**
	 ...
	 */
	function nandi_show_title() {
		return apply_filters( 'nandi_show_title', true );
	}
}

if ( ! function_exists( 'nandi_padding_css' ) ) {
	/**
	 ...
	 */
	function nandi_padding_css( $top, $right, $bottom, $left ) {
...
if ( ! function_exists( 'nandi_get_link_url' ) ) {
	/**
	 * Return the post URL.
	 *
...
if ( ! function_exists( 'nandi_get_navigation_location' ) ) {
	/**
	 ...
	 */
	function nandi_get_navigation_location() {
		return apply_filters( 'nandi_navigation_location', nandi_get_setting( 'nav_position_setting' ) );
	}
}
