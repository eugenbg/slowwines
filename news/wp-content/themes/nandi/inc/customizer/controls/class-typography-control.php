...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Nandi_Typography_Customize_Control' ) ) {
	/**
	 * Create the typography elements control.
	 *
	 */
	class Nandi_Typography_Customize_Control extends WP_Customize_Control {
		public $type = 'nandi-customizer-typography';

		public function enqueue() {
			wp_enqueue_script( 'nandi-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/selectWoo.min.js', array( 'customize-controls', 'jquery' ), NANDI_VERSION, true );
			wp_enqueue_style( 'nandi-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/selectWoo.min.css', array(), NANDI_VERSION );

			wp_enqueue_script( 'nandi-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/typography-customizer.js', array( 'customize-controls', 'nandi-typography-selectWoo' ), NANDI_VERSION, true );
			wp_enqueue_style( 'nandi-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/typography-customizer.css', array(), NANDI_VERSION );
		}

		public function to_json() {
			parent::to_json();

			$number_of_fonts = apply_filters( 'nandi_number_of_fonts', 200 );
			$this->json[ 'default_fonts_title'] = __( 'System fonts', 'nandi' );
			$this->json[ 'google_fonts_title'] = __( 'Google fonts', 'nandi' );
			$this->json[ 'google_fonts' ] = apply_filters( 'nandi_typography_customize_list', nandi_get_all_google_fonts( $number_of_fonts ) );
			$this->json[ 'default_fonts' ] = nandi_typography_default_fonts();
			$this->json[ 'family_title' ] = esc_html__( 'Font family', 'nandi' );
			$this->json[ 'weight_title' ] = esc_html__( 'Font weight', 'nandi' );
			$this->json[ 'transform_title' ] = esc_html__( 'Text transform', 'nandi' );
			$this->json[ 'category_title' ] = '';
			$this->json[ 'variant_title' ] = esc_html__( 'Variants', 'nandi' );
		...
		public function content_template() {
			...
			<# if ( 'undefined' !== typeof ( data.variant ) ) { #>
				...
				#>
				<div id={{{ data.variant.id }}}" class="nandi-font-variant" data-saved-value="{{ data.variant.value }}">
					<label>
			...
			<# if ( 'undefined' !== typeof ( data.category ) ) { #>
				<div class="nandi-font-category">
					<label>
							<input name="{{{ data.category.id }}}" type="hidden" {{{ data.category.link }}} value="{{{ data.category.value }}}" class="nandi-hidden-input" />
			...
			<# if ( 'undefined' !== typeof ( data.transform ) ) { #>
				<div class="nandi-font-transform">
					<label>
