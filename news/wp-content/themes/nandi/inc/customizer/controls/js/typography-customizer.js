( function( api ) {
	...
	api.controlConstructor['nandi-customizer-typography'] = api.Control.extend( {
		ready: function() {
			var control = this;

			control.container.on( 'change', '.nandi-font-family select',
				function() {
					...
					setTimeout( function() {
						// Send our request to the nandi_get_all_google_fonts_ajax function
						var response = jQuery.getJSON({
							type: 'POST',
							url: ajaxurl,
							data: {
								action: 'nandi_get_all_google_fonts_ajax',
								nandi_customize_nonce: nandi_customize.nonce
							},
							async: false,
							dataType: 'json',
						});
						...
						if ( id in fonts ) {
							...
							jQuery( '.nandi-font-family select' ).not( _this ).each( function( key, select ) {
								var parent = jQuery( this ).closest( '.nandi-font-family' );

								if ( _value == jQuery( select ).val() && _this.data( 'category' ) !== jQuery( select ).data( 'category' ) ) {
									if ( ! got_variants ) {
										updated_variants = jQuery( parent.next( '.nandi-font-variant' ).find( 'select' ) ).val();
										got_variants = true;
									}
								}
							} );
							...
							_this.closest( '.nandi-font-family' ).next( 'div' ).show();
							...
							_this.closest( '.nandi-font-family' ).next( 'div' ).hide();
			...
			control.container.on( 'change', '.nandi-font-variant select',
				function() {
					var _this = jQuery( this );
					...
					jQuery( '.nandi-font-variant select' ).each( function( key, value ) {
						var this_control = jQuery( this ).closest( 'li' ).attr( 'id' ).replace( 'customize-control-', '' );
						var parent = jQuery( this ).closest( '.nandi-font-variant' );
			...
			control.container.on( 'change', '.nandi-font-category input',
				function() {
					control.settings['category'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.nandi-font-weight select',
				function() {
					control.settings['weight'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.nandi-font-transform select',
				function() {
...
jQuery( document ).ready( function($) {
	...
	jQuery( '.nandi-font-family select' ).selectWoo();
	jQuery( '.nandi-font-variant' ).each( function( key, value ) {
		var _this = $( this );
	...
	$( ".nandi-font-family" ).each( function( key, value ) {
		var _this = $( this );
		if ( $.inArray( _this.find( 'select' ).val(), typography_defaults ) !== -1 ) {
			_this.next( '.nandi-font-variant' ).hide();
		}
	});
