...
function qem_process_values($values,$id) {
    ...
    $quantity = (float) ($values['items'][0]['quantity'] < 1 ? '1' : strip_tags($values['items'][0]['quantity']));
   	...
   	if ($qpp['useprocess'] && $qpp['processpercent']) {
        $percent = preg_replace ( '/[^.,0-9]/', '', $qpp['processpercent']) / 100;
        $handlingpercent = $amount * $quantity * $percent;
		$handling_percent = $percent;
        $handlingpercent = (float) qpp_format_amount($currency[$id],$qpp,$handling);
    }
	if ($qpp['useprocess'] && $qpp['processfixed']) {
        $handlingfixed = preg_replace ( '/[^.,0-9]/', '', $qpp['processfixed']);
        $handlingfixed = (float) qpp_format_amount($currency[$id],$qpp,$handling);
    }
	
    $handling = $handlingpercent + $handlingfixed;
...
<input type="hidden" name="thesum" value="' . strip_tags($values['thesum']) . '" /></p>';
                ...
                if ($register['useblank1']) {
                    ...
                    if ($register['yourblank1textarea']) $content .= '<textarea rows="4" label="blank1" name="yourblank1" '.$required.' '.$errors['yourblank1'].' onblur="if (this.value == \'\') {this.value = \''.$values['yourblank1'].'\';}" onfocus="if (this.value == \''.$values['yourblank1'].'\') {this.value = \'\';}" />' . stripslashes($values['yourblank1']) . '</textarea>';
                    else $content .= '<input id="yourblank1" name="yourblank1" '.$required.' '.$errors['yourblank1'].' type="text" value="'.$values['yourblank1'].'" onblur="if (this.value == \'\') {this.value = \''.$values['yourblank1'].'\';}" onfocus="if (this.value == \''.$values['yourblank1'].'\') {this.value = \'\';}" />';
                }
                break;
                case 'field10':
                if ($register['useblank2']) {
                    $required = ($register['reqblank2'] ? 'class="required"' : '');
                    if ($register['yourblank2textarea']) $content .= '<textarea rows="4" label="blank2" name="yourblank2" '.$required.' '.$errors['yourblank2'].' onblur="if (this.value == \'\') {this.value = \''.$values['yourblank2'].'\';}" onfocus="if (this.value == \''.$values['yourblank2'].'\') {this.value = \'\';}" />' . stripslashes($values['yourblank2']) . '</textarea>';
                    else $content .= '<input id="yourblank2" name="yourblank2" '.$required.' '.$errors['yourblank2'].' type="text" value="'.$values['yourblank2'].'" onblur="if (this.value == \'\') {this.value = \''.$values['yourblank2'].'\';}" onfocus="if (this.value == \''.$values['yourblank2'].'\') {this.value = \'\';}" />';
                }
                break;
                case 'field11':
                ...
                if ($register['usenumber1']) {
                    ...
                    $content .= '<p>'.$register['yournumber1'].'&nbsp;<input id="yournumber1" name="yournumber1" '.$required.' '.$errors['yournumber1'].' type="text" style="'.$errors['yournumber1'].'width:3em;margin-right:5px" value="'.$values['yournumber1'].'" value="'.$values['yournumber1'].'" onblur="if (this.value == \'\') {this.value = \''.$values['yournumber1'].'\';}" onfocus="if (this.value == \''.$values['yournumber1'].'\') {this.value = \'\';}" /></p>';
                }
                break;
                case 'field13';
                if ($register['useaddinfo'] && ( ($paypal && $register['paypaladdinfo']) || (!$paypal && !$register['paypaladdinfo']) ) )
        ...
        if ($register['ignorepayment'] && ($paypal && $cost)) {
            $content .= '<p><input type="checkbox" id="paylater" name="ignore" value="checked" '.$values['ignore'].' />'.$register['ignorepaymentlabel'].'</p>';
        }    
        if ($paypal && $cost) {
			
            $button_value = $payment['qempaypalsubmit'];
            ...
            $button_value = $register['qemsubmit'];
        ...
        <input type="submit" value="'.$button_value.'" alt="'.$register['qemsubmit'].'" id="submit" name="qemregister'.$id.'" />
        </form></div>
...
function qem_send_confirmation ($auto,$values,$content,$register,$id) {
    ...
    if ($paymentlink && $payment['message']) $auto['message'] = $payment['message'];
...
function qem_build_event_message($values,$register) {
    ...
    foreach ($sort as $name) {
        ...
        break;
        case 'field6':
            if ($register['usemessage'] && $register['yourmessage'] != $values['yourmessage']) $content .= '<p><b>' . $register['yourmessage'] . ': </b>' . strip_tags(stripslashes($values['yourmessage'])) . '</p>';
        break;
        case 'field9':
            if ($register['useblank1'] && $register['yourblank1'] != $values['yourblank1']) $content .= '<p><b>' . $register['yourblank1'] . ': </b>'. strip_tags(stripslashes($values['yourblank1'])).'</p>';
        break;
        case 'field10':
            if ($register['useblank2'] && $register['yourblank2'] != $values['yourblank2']) $content .= '<p><b>' . $register['yourblank2'] . ': </b>' . strip_tags(stripslashes($values['yourblank2'])) . '</p>';
        break;
        case 'field11':
            if ($register['usedropdown']) {
                $arr = explode(",",$register['yourdropdown']);
        ...
        break;
        case 'field12':
            if ($register['usenumber1'] && $values['yournumber1']) $content .= '<p><b>' . $register['yournumber1'] . ': </b>' . strip_tags(stripslashes($values['yournumber1'])) . '</p>';
        break;
        }
    }
