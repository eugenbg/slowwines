...
. "Content-Type: text/html; charset=\"utf-8\"\r\n";	
        ...
        if (is_admin()) qem_admin_notice(__('Message sent to selected', 'quick-event-manager').'.');
    }

    if( isset($_POST['qem_email_all'])) {
        $event = $_POST["qem_download_form"];
        $message = get_option('qem_messages_'.$event);
        $count = count($message);
        $subject = $_POST["subject"];
...
. "Content-Type: text/html; charset=\"utf-8\"\r\n";	
        ...
        if (is_admin()) qem_admin_notice(__('Message sent to all registrants', 'quick-event-manager').'.');
    }
    ...
    $dashboard = '<input type="submit" name="qem_email_selected" class="button-secondary" value="Send message to Selected"/> <input type="submit" name="qem_email_all" class="button-secondary" value="Send message to All"/>';
    $dashboard .= '</form>';
}
...
function qem_extend_create_list_table ($register,$emails,$sortby) {
    foreach ($emails as $item) {
        if ($item[0]) {
            if ($item[1]) $dash = ' - ';
            $order = ($sortby == 'name' ? $item[1].$dash.$item[0] : $item[0].$dash.$item[1]);
            $table .= '<h2>'.$order.'</h2>
            ...
            $table .= '</tr>';
            $content = qem_extend_build_registrations($register,$emails);
            $table .= $content;
            $table .= '</table>'; 
        }
    }
...
. "Content-Type: text/html; charset=\"utf-8\"\r\n";	
    ...
    if (!$qem_email) {
        $dashboard .= '<p>Enter your email address below to find out what events you are registered for:</p>
    <div class="qem-register">
    <input type="radio" name="showevents" id="allevents" value="all" ' . $all . ' /> <label for = "allevents">All Events</label> <input type="radio" name="showevents" id="currentevents" value="current" ' . $current . ' checked = "checked" /> <label for = "currentevents">Current Events</label><input type="text" name="youremail" value=""><input type="submit" name="qem_get_email" id="submit" value="Submit" />
    </div>';
    
    } else {
        $emails = array(array($qem_email,$yourname,$showevents));
        $content .= qem_extend_create_list_table ($register,$emails,'name');
        if ($content) {
            $dashboard .= '<style>#qem-widget table {margin-bottom: 20px;}#qem-widget td {border-top: 1px solid #005F6B;padding: 5px 10px 5px 0;max-width: 400px;vertical-align:top;}#qem-widget td b {color: #005F6B;}#qem-widget th {padding: 5px 10px 5px 0;max-width: 400px;text-align: left;color: #005F6B;}</style>
            <h2>Your Registrations</h2>'.$content.'<p>Send to this email address: <input type="text" name="youremail" value="'.$qem_email.'" >&nbsp;<input type="submit" name="qem_emaillist" class="button-primary" value="Send" /></p></form></div>';
        } else {
            $dashboard = '<h2>No registrations found</h2>';
        }
    }
    return $dashboard;
}

function qem_extend_build_registrations($register,$emails) {
    $args = array('post_type'=> 'event','orderby'=>'title','order'=>'ASC','posts_per_page'=> -1);
            ...
            if ($message && $checkyear) {
                foreach ($message as $item) {
                    ...
                    if ($item['youremail'] == $emails[0][0]) {
                        ...
                        if($item['yourmessage'] == $register['yourmessage']) $item['yourmessage']='';
...
function qem_extend_show_report ($atts) {
    ...
    if ($atts['listby'] == 'date') $listby = 'date';
    $register = qem_get_stored_register();
    if( isset($_POST['qem_emaillist'])) {
        $content = qem_extend_build_report($register,$eventid,$yearnumber,$listby);
        $qem_email = $_POST['youremail'];
        $content = '<html><style>th {padding: 5px;text-align: left;}
        </style>'.$content.'</html>';
        $headers = "From: {<{$qem_email}>\r\n"
...
. "Content-Type: text/html; charset=\"utf-8\"\r\n";	
    ...
    <div id="qem-widget">
    <form method="post" id="qem_download_form" action="">';
    $dashboard .= qem_extend_build_report($register,$eventid,$yearnumber,$listby);
...
function qem_extend_build_report($register,$eventid,$yearnumber,$listby) {
    if ($listby == 'date') 
    ...
    if ( have_posts()){
        while (have_posts()) {
            ...
            if (empty($eventid) || in_array($id,$eventid)) {
                ...
                $unixtime = get_post_meta($id, 'event_date', true);
                $date = date_i18n("d M Y", $unixtime);
                if (($yearnumber && $yearnumber == date("Y", $unixtime)) || !$yearnumber) $message = get_option('qem_messages_'.$id);   
            
                if ($message) {
                    if ($listby == 'date') $dashboard .= '<h3>'.$date.' - '.$title.'</h3>';
                    else $dashboard .= '<h3>'.$title.' - '.$date.'</h3>
                    <p>Event ID: '.$id.'</p>';
...
function qem_extend_admin_extension_pages() {
    add_submenu_page('edit.php?post_type=event' , __('Reports', 'quick-event-manager') , __('Reports', 'quick-event-manager') , 'edit_events' ,'reports' , 'qem_tabbed_extend');
}
