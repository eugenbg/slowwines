...
Description: A simple event manager. There is nothing to configure, all you need is an event and a shortcode.
Version: 8.9
Author: aerin
...
add_shortcode('qemsendemail', 'qem_extend_registration_send_email');
...
function qem_event_shortcode($atts,$widget) {
    ...
    if ($display['categorydropdown']) $content .= qem_category_dropdown($display);
    
    if (!$widget && $display['cat_border'] && ($display['showkeyabove'] || $atts['categorykeyabove'])) $content .= qem_category_key($cal,$style,'');
    ...
    if ($atts['id'] == 'nextweek') $nextweek = strtotime("+7 day", $today);
    ...
    if ( $the_query->have_posts()){
        ...
        if (!$widget && $display['cat_border'] && ($display['showkeybelow'] || $atts['categorykeyabove'])) $content .= qem_category_key($cal,$style,'');
        if ($widget && $atts['usecategory'] && $atts['categorykeyabove']) $content .= qem_category_key($cal,$style,'');
        if ($atts['listlink']) $content .= '<p><a href="'.$atts['listlinkurl'].'">'.$atts['listlinkanchor'].'</a></p>';

        echo $content;
        if (!$atts['widget']) echo wp_qemnavi(array('query'=>$the_query));
    }
...
function qem_category_dropdown($display) {
    ...
    foreach($arr as $option) {
        $thecat = $option->name;
        $selected = '';
        if ($values['category'] == $thecat) $selected = 'selected';
...
function qem_event_construct ($atts) { 
    ...
    if(strtolower($atts['links']) == 'off') {$display['titlelink']=$display['readmorelink'] = true;}
    ...
    switch ( $name ) {
                ...
                if ($register['listnames']) {
                    foreach($whoscoming as $item) {
                        ...
                        (($register['moderate'] && $item['approved']) || !$register['moderate'])) {
                            ...
                            if ($url) $url = ' <a href="'.$item['yourblank1'].'">'.$url.'</a>';
                            
                            $msg = $register['listblurb'];
                            $msg = str_replace('[name]', $item['yourname'],$msg);
                            $msg = str_replace('[email]', $item['youremail'],$msg);
                            $msg = str_replace('[mailto]', '<a href="mailto:'.$item['youremail'].'">'.$item['youremail'].'</a>',$msg);
                            $msg = str_replace('[places]', $item['yourplaces'],$msg);
                            $msg = str_replace('[telephone]', $item['yourtelephone'], $msg);
                            $msg = str_replace('[user1]', $item['yourblank1'], $msg);
                            $msg = str_replace('[user2]', $item['yourblank2'], $msg);
                            $msg = str_replace('[website]', $url, $msg);
                            if ($item['yourname']) $str = $str.'<li>'.$msg.'</li>';
                        }
                    }
                    if ($str && $event['whoscoming']) $content .= '<p id="whoscoming" ' . $style . '>'.$event['whoscomingmessage'].'</p><ul>'.$str.'</ul>';
                } else {
                    foreach($whoscoming as $item) {
                        $num = $num + $item['yourplaces'];
                        $ipn = qem_check_ipnblock($payment,$item);
                    ...
                    $str = substr($str, 0, -2);
                    if ($str && $event['whoscoming']) $content .= '<p id="whoscoming" ' . $style . '>'.$event['whoscomingmessage'].' '.$str.'</p>';
        ...
        case 'field11':
            ...
            if ($register['waitinglist'] && $num == 0 && $usecounter) {
                ...
                $output .= '<p class="placesavailable" ' . $style . '>'.qem_places($register,$post->ID,$usecounter,$event).'</p>';
            }
        break;
        ...
        case 'field14':
            ...
            if ($event['facebook_label']) {
                ...
                <script>(function(d, s, id) {var js, fjs = d.getElementsByTagName(s)[0];if (d.getElementById(id)) return;js = d.createElement(s); js.id = id;js.src = "//connect.facebook.net/en_US/sdk.js#xfbml=1&version=v2.8&appId=205093296628825";fjs.parentNode.insertBefore(js, fjs);}(document, \'script\', \'facebook-jssdk\'));</script>
                <a style="background:#3B5998;" target="_blank" OnClick="window.open(this.href,\'targetWindow\',\'titlebar=no,toolbar=no,location=0,status=no,menubar=no,scrollbars=yes,resizable=yes,width=600,height=250\'); return false;" href="https://www.facebook.com/sharer/sharer.php?u='.rawurlencode($link).'">'.$event['facebook_label'].'</a>'; 
            }
            if ($event['twitter_label']) {
                ...
                $output .='&nbsp;<a style="background:#1DA1F2;" target="_blank" OnClick="window.open(this.href,\'targetWindow\',\'titlebar=no,toolbar=no,location=0,status=no,menubar=no,scrollbars=yes,resizable=yes,width=600,height=250\'); return false;" href="https://twitter.com/share?url='.$link.'&text='.$date.'+-+'.$title.'&hashtags=WFTR">'.$event['twitter_label'].'</a><script async src="https://platform.twitter.com/widgets.js" charset="utf-8"></script> ';
            }
            if ($event['useicsbutton'] && (!$display['useics'] || !$display['uselistics'])) {
...
function qem_get_the_numbers($pid,$payment) {
    ...
    if ($whoscoming)
        foreach($whoscoming as $item) {
            if (!qem_check_ipnblock($payment,$item) && !isset($item['notattend']) && (($register['moderate'] && isset($item['approved'])) || !$register['moderate'] ||($register['moderate'] && $register['moderateplaces']))) {
...
function qem_places ($register,$pid,$usecounter,$event = array()) {
...
function qem_show_calendar($atts) {
    ...
    $currentmonth = filter_var($_REQUEST["qemmonth"], FILTER_SANITIZE_NUMBER_INT);
    $currentyear = filter_var($_REQUEST["qemyear"], FILTER_SANITIZE_NUMBER_INT);
    ...
    for ($i=0; $i<($maxday+$startday); $i++) {
        ...
        foreach ($eventdate as $key => $day) {
            ...
            if($xxx==$zzz && $show) {
                ...
                $short = $length - 3;
                if ($cal['attendeeflag']) $numbers = ($eventnumbers[$key]) ? $flag.'&nbsp;' : '';
...
function qem_generate_css() {
    ...
    if (!isset($formwidth[0])) $formwidth[0] = '280';
...
@media only screen and (max-width: 480px) {.qem-large, .qem-medium {margin-left: 50px;}
    ...
    if ($style['vanilla']) $script .='.qem h2, .qem h3 {display:block;}';
    if ($cal['tdborder']) {
...
@media only screen and (max-width: 480px) {#xlightbox {width:90%;margin-left:-45%;}}';
    ...
    foreach ($cat as $i) {
        if ($style['cat'.$i]) {
            ...
            if ($style['date_background'] == 'category') $script .='.'.$style['cat'.$i].' .qem-calendar-small .day, .'.$style['cat'.$i].' .qem-calendar-medium .day, .'.$style['cat'.$i].' .qem-calendar-large .day {background:'.$style['cat'.$i.'back'].';color:'.$style['cat'.$i.'text'].';}';
        }
    }
...
div.toggle-qem a:link, div.toggle-qem a:visited, div.toggle-qem a:hover {color:#FFF;text-decoration:none !important;}';
        ...
        if ($register_style['header-colour']) $headercolour = "color: ".$register_style['header-colour'].";";

        $header = ".qem-register ".$hd." {".$headercolour.";height:auto;}";

        $input = '.qem-register input[type=text], .qem-register textarea, .qem-register select {color:'.$register_style['font-colour'].';border:'.$register_style['input-border'].';background:'.$register_style['inputbackground'].';line-height:normal;height:auto;margin: 2px 0 3px 0;padding: 6px;}';

        $required = '.qem-register input[type=text].required, .qem-register textarea.required, .qem-register select.required {border:'.$register_style['input-required'].'}'; 
        $focus = ".qem-register input:focus, .qem-register textarea:focus {background:".$register_style['inputfocus'].";}";

        $text = ".qem-register p {color:".$register_style['font-colour'].";margin: 6px 0 !important;padding: 0 !important;}";
        ...
        if ($register_style['border']<>'none') $border =".qem-register #".$register_style['border']." {border:".$register_style['form-border'].";}";
        if ($register_style['background'] == 'white') $background = ".qem-register div {background:#FFF;}";
        if ($register_style['background'] == 'color') $background = ".qem-register div {background:".$register_style['backgroundhex'].";}";
        ...
        if ($register_style['submitwidth'] == 'submitpercent') $submitwidth = 'width:100%;';
        if ($register_style['submitwidth'] == 'submitrandom') $submitwidth = 'width:auto;';
        if ($register_style['submitwidth'] == 'submitpixel') $submitwidth = 'width:'.$style['submitwidthset'].';';
        if ($register_style['submitposition'] == 'submitleft') $submitposition = 'float:left;'; else $submitposition = 'float:right;';
...
function qem_time ($starttime) {
    ...
    $hours = $minutes = $afternoon = $needle = $minutes = $afternoon = false;
...
function broken_qem_time ($starttime) {
    
    $hours = $minutes = $afternoon = false;
    
    if (strpos(strtoupper($starttime),'PM') && substr($starttime, 0, 2) != '12') {
        $afternoon = 43200;
    }
    $starttime = (int) filter_var($starttime, FILTER_SANITIZE_NUMBER_INT);
    $hours = $starttime;
    if (strlen($starttime) == 4 && is_numeric($starttime)) {$hours = substr($starttime, 0, 2);$minutes = substr($starttime, 2);}
    $seconds=$hours*3600+$minutes*60+$afternoon;
	return $seconds;
...
function qem_ics_button($id,$label) {
	return "<a class='qem_ics_button' href='".admin_url( 'admin-ajax.php?action=qem_download_ics&id='.$id )."' target='_blank'>".$label."</a>";
}
...
function qem_ipn () {
	...
	if (!defined("LOG_FILE")) define("LOG_FILE", "./ipn.log");
	...
	$myRequest = ['cmd' => '_notify-validate'];
    ...
    foreach ($myPost as $key => $value) {
        if($get_magic_quotes_exists == true && get_magic_quotes_gpc() == 1) {
			...
			$myRequest[$key] = stripslashes($value);	   
        } else {
	...
	$http = new WPHttp($paypal_url);
	
	$http->SetHeader('Connection','Close');
	
	$http->SetArg('user-agent', 'QPP');
	$http->SetArg('timeout', '30');
	$http->SetArg('sslverify',true);
	$http->SetArg('httpversion','1.1');
	$http->SetArg('blocking',true);
	
	$http->SetMethod('POST');
	
	$http->SetBody($myRequest);
	
	$http->Execute();
	
	if ($http->success) {
		if(DEBUG == true) {
			error_log(date('[Y-m-d H:i e] '). "HTTP request of validation request:". implode('&',$http->return_headers) ." for IPN payload: $req" . PHP_EOL, 3, LOG_FILE);
...
function qem_upgrade_ipn() {
    $qemkey = get_option('qpp_key');
    if ( (!isset( $_REQUEST['paypal_ipn_result']) && !isset($_POST['custom'])) || $qemkey['authorised'] || $qemkey['dismiss'])
        return;
    define("DEBUG", 0);
...
. "Content-Type: text/html; charset=\"utf-8\"\r\n";	
        $message = '<html><p>Thank for upgrading. Your Quick Plugins authorisation key is:</p><p>'.$qemkey['key'].'</p></html>';
...
$display = event_get_stored_display();
if ($display['recentposts']) add_action( 'pre_get_posts', 'qem_add_custom_post_type_to_query' );
...
function qem_enqueue_scripts() {
	...
	switch ($ic['useapi']) {
		case 'paypal':
			wp_register_script( 'paypal_checkout', '//www.paypalobjects.com/api/checkout.js', array(), false, true);
		break;
		case 'stripe':
			wp_register_script( 'stripe_checkout', '//checkout.stripe.com/checkout.js', array(), false, true);
		break;
	}
...
function qem_get_next($rules,$current_date) {
	...
	// Set frequency to 'Every' for targets other than weekdays
	if (in_array($target,$every)) $frequency = $frequencies[0];
	...
	switch ($target) {
		...
		default: // week day
			...
			if ($frequency != 'Every') {
				...
				*/
				if (strtotime($string.' '.$month,$current_date) <= $current_date) $month = 'OF NEXT MONTH';
				$string = $string . ' ' . $month;
			} else {
				$string = 'Next '.$string;
			}
...
function qem_get_end($rules) {
	...
	return strtotime("+".$rules['number']." ".$rules['for'],$rules['start']);
...
function qem_create_post($date,$post_id,$publish,$duplicate) {
	...
	$end_diff = $postmeta['event_end_date'][0] - $postmeta['event_date'][0];
	$end_date = (($end_diff <= 0)? '':$date + $end_diff);
	$postmeta['event_date'][0] = $date;
