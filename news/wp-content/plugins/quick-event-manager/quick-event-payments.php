...
function qem_process_payment_form($values,&$val = array()) {
    ...
    if ($useproducts) {
        ...
        $quantity = 1;
        $cost = $values['qtyproduct0'] * preg_replace ( '/[^.0-9]/', '', $products[2]) + $values['qtyproduct1'] * preg_replace ( '/[^.0-9]/', '', $products[4]) + $values['qtyproduct2'] * preg_replace ( '/[^.0-9]/', '', $products[6]) + $values['qtyproduct3'] * preg_replace ( '/[^.0-9]/', '', $products[8]);
    }
    ...
    if ($payments['useprocess'] && $payments['processpercent']) {
        $percent = preg_replace ( '/[^.,0-9]/', '', $payments['processpercent']) / 100;
        $percentprocess .= $cost * $quantity * $percent;
    }
    
    if ($payments['useprocess'] && $payments['processfixed']) {
