...
function cprp_admin_init(){
        ...
        switch( strtolower( $_REQUEST[ 'cprp-action' ] ) ){
            case 'extract-all':
                ...
                if( count( $results ) ){
                    ...
                    print '<div style="text-align:center;"><h1>'.__( 'Processing Posts', 'cp-related-posts' ).'</h1></div>';
                ...
                print '<div style="text-align:center;"><h1>'.__( 'All Posts Processed', 'cp-related-posts' ).'</h1></div>';
                exit;
            ...
            case 'extract-tags':
                ...
                if( isset( $_REQUEST[ 'id' ] ) && isset( $_REQUEST[ 'text' ] ) ){
					...
					{
						$tags = $cprp_tags_obj->get_tags( $post->post_title.' '.$post->post_content );
					}
					else
					{
    ...
    $form_title = __('Select the posts related and tags proposed', 'cp-related-posts');
...
?>
	...
	<?php _e('For any question, go to the <a href="https://wordpress.org/support/plugin/cp-related-posts#new-post" target="_blank">support</a> and leave us a message.'); ?><br/><br />
	...
	<?php _e('To get the premium version of CP Related Posts go to the following links:<br/> <a href="http://wordpress.dwbooster.com/content-tools/related-posts#download" target="_blank">CLICK HERE</a>'); ?><br/><br />
...
<?php
    if (isset($_POST['cprp_settings']) && wp_verify_nonce( $_POST['cprp_settings'], plugin_basename( __FILE__ ) ) ){
        ...
        if( !empty($_REQUEST[ 'cprp_number_of_posts' ]) && @is_numeric(trim( $_REQUEST[ 'cprp_number_of_posts' ]))) $settings[ 'number_of_posts' ] = intval($_REQUEST[ 'cprp_number_of_posts' ]);
        if(!empty($_REQUEST[ 'cprp_similarity' ]) && @is_numeric(trim( $_REQUEST[ 'cprp_similarity' ]))) $settings[ 'similarity' ] = intval($_REQUEST[ 'cprp_similarity' ]);
    ...
    <form method="post" action="<?php print esc_attr(cprp_site_url()).'admin.php?page=cprp-settings';?>">
        <div class="postbox" style="margin: 5px 15px;">
            <h3 class='hndle' style="padding:5px;"><span><?php _e( 'Related Posts Settings', 'cp-related-posts' ); ?></span></h3>
            <div class="inside">
                <table class="form-table">
                    <tr valign="top">
                        <th><?php _e( 'Section title', 'cp-related-posts' ); ?></th>
                        <td>
                            <input type="text" name="cprp_title" value="<?php echo esc_attr( cprp_get_settings( 'title' ) ); ?>" style="width:150px;" />
                            <em><?php _e( 'The title of the related posts section', 'cp-related-posts' ); ?></em>
                        </td>
                    </tr>

                    <tr valign="top">
                        <th><?php _e( 'Number of related posts', 'cp-related-posts' ); ?></th>
                        <td>
                            <input type="text" name="cprp_number_of_posts" value="<?php echo esc_attr( cprp_get_settings( 'number_of_posts' ) ); ?>" style="width:150px;" />
                            <em><?php _e( 'Number of posts to display as related posts', 'cp-related-posts' ); ?></em>
                        </td>
                    </tr>

                    <tr valign="top">
                        <th><?php _e( 'Post types that allow related posts', 'cp-related-posts' ); ?></th>
                        <td>
                            <?php
                            ...
                            <em><?php _e( 'Select the posts types that will display related posts', 'cp-related-posts' ); ?></em>
					...
					<tr valign="top">
                        <th><?php _e( 'Display percentage of similarity with the symbol', 'cp-related-posts' ); ?></th>
                        <td>
					<?php
						foreach( $list_of_symbols as $symbol )
						{
							...
							for( $i = 0; $i < 3; $i++) echo '<img src="'.esc_attr($images_dir.$symbol.'_on.png').'" />';
							for( $i = 0; $i < 2; $i++) echo '<img src="'.esc_attr($images_dir.$symbol.'_off.png').'" />';
							echo '<br />';
                        }
					?>
					...
					<tr valign="top">
                        <th><?php _e( 'Display related posts with a percentage of similarity bigger than', 'cp-related-posts' ); ?></th>
                        <td>
                ...
                <div style="border: 1px solid #CCC; padding: 10px;" >
                    ...
                    <table class="form-table">
                        <tr>
                            <td colspan="2">
                                <h2><?php _e( 'How to display related posts in single pages', 'cp-related-posts' ); ?></h2>
                            </td>
                        </tr>
                        <tr valign="top">
                            <th><?php _e( 'Display related posts in single pages', 'cp-related-posts' ); ?></th>
                            <td>
                        ...
                        <tr valign="top">
                            <th><?php _e( 'Display featured images in related posts', 'cp-related-posts' ); ?></th>
                            <td>
                        ...
                        <tr valign="top">
                            <th><?php _e( 'Size of featured images', 'cp-related-posts' ); ?></th>
                            <td>
                        ...
                        <tr valign="top">
                            <th><?php _e( 'Display percentage of similarity', 'cp-related-posts' ); ?></th>
                            <td>
                        ...
                        <tr valign="top">
                            <th><?php _e( 'Display excerpt of related posts', 'cp-related-posts' ); ?></th>
                            <td>
                        ...
                        <tr valign="top">
                            <th><?php _e( 'Number of words on posts excerpts', 'cp-related-posts' ); ?></th>
                            <td>
                        ...
                        <tr valign="top">
                            <th><?php _e( 'Display related terms between related posts', 'cp-related-posts' ); ?></th>
                            <td>
                        ...
                        <tr valign="top">
                            <th><?php _e( 'Display mode', 'cp-related-posts' ); ?></th>
                            <td>
                ...
                <div style="border: 1px solid #CCC;margin-top:10px;padding:10px;" >
                    ...
                    <table class="form-table">
                        <tr>
                            <td colspan="2">
                                <h2><?php _e( 'How to display related posts in multiple-posts pages', 'cp-related-posts' ); ?></h2>
                            </td>
                        </tr>
                        <tr valign="top">
                            <th><?php _e( 'Display related posts in multiple-posts pages', 'cp-related-posts' ); ?></th>
                ...
                <div class="submit">
                    ...
                    <a class="button" href="<?php print esc_url( cprp_site_url().'?cprp-action=extract-all' ); ?>" target="_blank"><?php _e( 'Process Previous Posts' ); ?></a>
                </div>
            </div>
        </div>
	</form>
...
?>
    ...
    <p><?php _e( 'After complete the post writing, press the "Get recommended tags" button to get a list of possible tags determined by content, and select the most relevant tags', 'cp-related-posts' ); ?></p>
    ...
    <p><?php _e( 'If there is a post or page related directly to the current article, then it is possible associate both items manually. Search the related article, and press the "+" symbol.', 'cp-related-posts' ); ?></p>
    ...
    <div id="cprp_manually_added"><div class="cprp-section-title"><?php _e( 'Items manually related, press "-" symbol to remove item', 'cp-related-posts' ); ?></div><div class="cprp-container"><ul>
    <?php
    ...
    <div id="cprp_found"><div class="cprp-section-title"><?php _e( 'Items found, press "+" symbol to associate the item', 'cp-related-posts' ); ?></div><div class="cprp-container"></div></div>
<?php
...
function cprp_customization_links( $links ){
	array_unshift(
        $links,
        '<a href="'.esc_attr(cprp_site_url()).'admin.php?page=cprp-settings">'.__( 'Settings', 'cp-related-posts' ).'</a>',
        '<a href="http://wordpress.dwbooster.com/contact-us" target="_blank">'.__( 'Request custom changes', 'cp-related-posts' ).'</a>',
        '<a href="https://wordpress.org/support/plugin/cp-related-posts#new-post" target="_blank">'.__( 'Help', 'cp-related-posts' ).'</a>'
        );
    return $links;
...
function _cprp_content( $the_content, $mode = '' ){
    ...
    if( !empty( $tags_arr ) ){
        ...
        $query = "SELECT posts.*, postmeta.meta_value FROM $wpdb->posts as posts, $wpdb->postmeta as postmeta WHERE posts.post_type IN ('".implode( "','", $post_types )."') AND posts.post_status='publish' AND posts.ID = postmeta.post_id AND posts.ID <> ".$post->ID." AND postmeta.meta_key = 'cprp_tags'"
    ...
    if( count( $related_posts ) ){
        ...
        $str .= '<div class="cprp_items '.esc_attr($mode).'"><ul>';
        ...
        for( $i = 0; $i < $h; $i++ ){
            ...
            if ( $display[ 'show_thumbnail' ] && has_post_thumbnail( $related_posts[ $i ]->ID ) ){
                ...
                $thumb = apply_filters(
							'cprp_post_thumbnail',
							'<a href="'.esc_url($link).'"><img src="'.esc_url($image[ 0 ]).'" class="cprp_thumbnail" /></a>',
							$related_posts[ $i ]
						);
            }
            ...
            $str .= apply_filters(
						'cprp_post_title',
						'<div class="cprp_title"><a href="'.esc_url($link).'">'.wp_strip_all_tags($related_posts[ $i ]->post_title, true).'</a></div>',
						$related_posts[ $i ]
					);
